  ...
  try {
    ...
    const html = `
      <div style="font-family: 'Segoe UI', sans-serif; background-color: #f9fafb; padding: 20px; border-radius: 10px; border: 1px solid #e5e7eb;">
        ...
        <p>Hello,</p>
        <p>Your One-Time Password (OTP) for <strong>Writeer</strong> is:</p>
  ...
  try {
    ...
    const html = `
      <div style="font-family: 'Segoe UI', sans-serif; background-color: #f9fafb; padding: 20px; border-radius: 10px; border: 1px solid #e5e7eb;">
        ...
        <p>Hello,</p>
        <p>We received a request to reset your <strong>Writeer</strong> account password.  
        Use the One-Time Password (OTP) below to verify your request:</p>
        <div style="text-align: center; margin: 20px 0;">
          <span style="display: inline-block; background-color: #4f46e5; color: white; font-size: 22px; letter-spacing: 4px; padding: 10px 20px; border-radius: 8px;">
            ${otp}
          </span>
        </div>
        <p>This code will expire in <strong>5 minutes</strong>. If you did not request a password reset, ignore this email.</p>
        <p style="font-size: 14px; color: #9ca3af; text-align: center;">
          © ${new Date().getFullYear()} Writeer. All rights reserved.
        </p>
      </div>
    ...
    await sendEmail(email, "Writeer Password Reset Code – Secure Your Account", html);
